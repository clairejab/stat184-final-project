...
Renaming variables so it is easier to understand data table.Preview of new data table called TidyThanksgiving Data which has updated headings for major variables.
```{r}
...
Grouping by sex shows that there were 544 women who completed the survey and 481 males that completed the survey. The 33 number that has no category shows people who may have fogotten to fill out gender or those who did not feel the need to share their gender. These 33 who did not share their gender were removed from the data table.
```{r}
TidyThanksgivingData %>%
  group_by(Sex) %>%
  filter(Sex != "") %>%
